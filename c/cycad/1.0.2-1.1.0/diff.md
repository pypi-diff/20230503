# Comparing `tmp/cycad-1.0.2.tar.gz` & `tmp/cycad-1.1.0.tar.gz`

## filetype from file(1)

```diff
@@ -1 +1 @@
-gzip compressed data, was "cycad-1.0.2.tar", last modified: Tue Oct 18 18:06:27 2022, max compression
+gzip compressed data, was "cycad-1.1.0.tar", last modified: Wed May  3 16:39:28 2023, max compression
```

## Comparing `cycad-1.0.2.tar` & `cycad-1.1.0.tar`

### file list

```diff
@@ -1,14 +1,14 @@
-drwxr-xr-x   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)        0 2022-10-18 18:06:27.903810 cycad-1.0.2/
--rw-r--r--   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)     1062 2022-08-04 21:55:59.000000 cycad-1.0.2/LICENSE
--rw-r--r--   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)      454 2022-10-18 18:06:27.903626 cycad-1.0.2/PKG-INFO
-drwxr-xr-x   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)        0 2022-10-18 18:06:27.898777 cycad-1.0.2/cycad/
-drwxr-xr-x   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)        0 2022-10-18 18:06:27.900326 cycad-1.0.2/cycad/src/
-drwxr-xr-x   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)        0 2022-10-18 18:06:27.903336 cycad-1.0.2/cycad/src/cycad.egg-info/
--rw-r--r--   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)      454 2022-10-18 18:06:27.000000 cycad-1.0.2/cycad/src/cycad.egg-info/PKG-INFO
--rw-r--r--   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)      229 2022-10-18 18:06:27.000000 cycad-1.0.2/cycad/src/cycad.egg-info/SOURCES.txt
--rw-r--r--   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)        1 2022-10-18 18:06:27.000000 cycad-1.0.2/cycad/src/cycad.egg-info/dependency_links.txt
--rw-r--r--   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)       59 2022-10-18 18:06:27.000000 cycad-1.0.2/cycad/src/cycad.egg-info/requires.txt
--rw-r--r--   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)        6 2022-10-18 18:06:27.000000 cycad-1.0.2/cycad/src/cycad.egg-info/top_level.txt
--rw-r--r--   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)    19010 2022-10-18 18:06:07.000000 cycad-1.0.2/cycad/src/cycad.py
--rw-r--r--   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)       38 2022-10-18 18:06:27.903874 cycad-1.0.2/setup.cfg
--rw-r--r--   0 yuewu    (1662288217) SLAC\Domain Users (1704612529)     1309 2022-10-18 18:06:09.000000 cycad-1.0.2/setup.py
...
### Comparing `cycad-1.0.2/LICENSE` & `cycad-1.1.0/LICENSE`

 * *Files identical despite different names*

### Comparing `cycad-1.0.2/cycad/src/cycad.py` & `cycad-1.1.0/cycad/src/cycad.py`
...
+                    return pd.read_csv(
+                        f, sep=delimiter, usecols=[0, 1], names=["x", "y"]
+                    )
...
+                            _.append(
+                                pd.DataFrame(
+                                    get_data(self.files[i]),
+                                    columns=[self.parse_filename(self.files[i])],
+                                )
+                            )
                         else:
...
+                            _.append(
+                                pd.DataFrame(
+                                    get_data(self.files[i]), columns=[self.files[i]]
+                                )
+                            )
                     except:
...
+                        _.append(
+                            pd.DataFrame(
+                                np.zeros(self.df.shape[0]) + 0.1,
+                                columns=[self.files[i]],
+                            )
+                        )
...
+                            temp.rename(
+                                columns={"y": self.parse_filename(self.files[i])},
+                                inplace=True,
+                            )
...
+        _ = pd.read_csv(
+            path, encoding=encoding, sep="\t", decimal=decimal, skiprows=skip
+        )["Ewe/V"]
...
-            _.append(subtracted)
-        return pd.concat([df['x']] + _, axis=1)
+        # Create a copy of the dataframe with the same index and columns
+        corrected_df = df.copy()
+
+        sub_fn = lambda col: col[start:end] - self.baseline_arPLS(col[start:end])
+
+        # Apply the baseline_arPLS function to each column (excluding the first one) and subtract it from the original data
+        corrected_df.iloc[:, 1:] = df.iloc[:, 1:].apply(sub_fn, axis=0)
+
+        return corrected_df
...
+                self.df[self.df.columns[1:]]
+                - self.df[self.df.columns[1:]].min().min()
+                + 1e-6
+            )
+
         try:
...
+            self.correlation_matrix = np.array(self.df.corr())
...
+            self.df_echem = pd.DataFrame(
+                resample(self.df_echem.T, self.df.shape[1] - 1)
+            ).T
+
+            # Calculate the distance matrix
+            self.correlation_matrix_ec = squareform(pdist(self.df_echem.T))
...
+                    hspace=0.0,
+                    wspace=0.0,
+                    width_ratios=[1, 1, 1, 1, 1, 0.2, 0.2],
...
+                    hspace=0.0,
+                    wspace=0.0,
+                    width_ratios=[1, 1, 1, 1, 1, 1, 0.2, 0.2],
+                    height_ratios=[1, 1, 1, 1, 1],
+                )
...
+                cutoff = np.quantile(
+                    np.reshape(self.correlation_matrix_ec, -1), echem_quantile
+                )
+                ax_main.contourf(
+                    self.correlation_matrix_ec,
+                    levels=[0, cutoff],
+                    colors=[(1, 0, 0, echem_alpha), (0, 0, 0, 0)],
+                    origin="lower",
+                )
...
+            ax_y.imshow(
+                self.df.iloc[start:end, 1:].T,
+                extent=[
+                    self.df[self.df.columns[0]][start],
+                    self.df[self.df.columns[0]][end],
+                    0,
+                    self.df.shape[1],
+                ],
+                norm=LogNorm(vmin=vmin, vmax=vmax),
...
+                fig.savefig(
+                    r"./plots/" + filename, transparent=False, facecolor="white"
+                )
...
### Comparing `cycad-1.0.2/setup.py` & `cycad-1.1.0/setup.py`
